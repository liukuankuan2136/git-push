...
exports.WORK_HOUR_TO_TASK_CATALOG = void 0;
exports.collectDailyTaskInput = collectDailyTaskInput;
// @AI-Begin D8E9F 20260809 @@claudeCode
const vscode = __importStar(require("vscode"));
const TaskTemplateFlow_1 = require("./TaskTemplateFlow");
const QuickPickFlow_1 = require("./QuickPickFlow");
/**
 * 工时类型 eleCode → 任务类型 eleCode 映射表。
 * 来源：用户提供的工时类型字典与任务类型对照。
 */
exports.WORK_HOUR_TO_TASK_CATALOG = {
    '24': '3', // 代码编写 → 编码工作
 ...
 */
async function collectDailyTaskInput(provider, cache, workspacePath, existingMapping, taskCreateMode) {
    ...
    if (existingMapping) {
        ...
        if (useMapped.value) {
            ...
            const picked = await (0, QuickPickFlow_1.pickDevProjectAndProduct)(provider, cache);
            if (!picked) {
                return undefined;
            }
    ...
    const matchedRegion = (0, QuickPickFlow_1.matchRegionFromTitle)(taskName, regions);
    if (matchedRegion) {
        ...
        if (confirm.value) {
            ...
            const picked = await (0, QuickPickFlow_1.pickRegion)(regions);
            if (!picked) {
                return undefined;
            }
            regionId = picked;
    ...
    if (provider.fetchWorkHourTypes) {
        ...
        if (types.length > 0) {
            const typePick = await vscode.window.showQuickPick(types.map((t) => {
                const mappedCatalog = exports.WORK_HOUR_TO_TASK_CATALOG[t.eleCode];
            ...
            // 自动映射
            taskWorkItemCatalog = exports.WORK_HOUR_TO_TASK_CATALOG[workHourTypeCode] ?? DEFAULT_TASK_WORK_ITEM_CATALOG;
        }
    }
    // ── Step 7: 输入运维工时 ──
    ...
    const confirmation = await vscode.window.showInformationMessage(`确认创建日常任务？\n\n${summary}`, { modal: true }, '确认创建');
    if (confirmation !== '确认创建') {
        return undefined;
    }
    return {
        taskInput,
...
//# sourceMappingURL=DailyTaskFlow.js.map
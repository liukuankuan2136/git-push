...
async function recordHours(provider, metadata, config) {
    ...
    if (metadata.todayWorkHour && provider.modifyWorkHour) {
        ...
        try {
            ...
            throw new Error(`工时更新失败：${error instanceof Error ? error.message : String(error)}。请确认${taskLabel}编号 "${metadata.task.code}" 是否正确。`);
        }
    }
        ...
        try {
            ...
            throw new Error(`工时登记失败：${error instanceof Error ? error.message : String(error)}。请确认${taskLabel}编号 "${metadata.task.code}" 是否正确。`);
        }
    }

...
import { DevOpsProvider, DevOpsTask, WorkHourRecord } from '../core/DevOpsProvider';
import {
...
export async function collectRegionCheckReport(
  ...
  const devPick = await vscode.window.showQuickPick(
    ...
    { title: '区域合规检查 — 选择研发项目', ignoreFocusOut: true }
  );
  ...
  const prodPick = await vscode.window.showQuickPick(
    ...
    { title: '区域合规检查 — 选择所属产品', ignoreFocusOut: true }
  );
  ...
  const tasks = await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: '正在拉取本周任务列表（所有人的）', cancellable: false },
    () => provider.fetchTasksByProduct!(devPick.value, prodPick.value)
  );
  if (tasks.length === 0) {
  ...
  await vscode.window.withProgress(
    ...
    async (progress, token) => {
      ...
      for (const task of tasks) {
        ...
        const checkResult = checkRegionCompliance(
          task.title,
          workContents,
          regionName,
          opsprojName
 ...
 */
async function outputReport(results: TaskCheckResult[], channel: vscode.OutputChannel): Promise<void> {
  ...
  for (const result of violations) {
    ...
    const primary = extractPrimaryLocation(task.opsprojName ?? '');
  ...
  const action = await vscode.window.showWarningMessage(
    `区域合规检查: ${violations.length}/${results.length} 个任务存疑。详见输出面板。`,
    { modal: false },
    '查看详情'
  );
  if (action === '查看详情') {
